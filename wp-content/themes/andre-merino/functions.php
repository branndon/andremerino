...
register_sidebar( array(
	...
	'id'			=> 'footer',
	'description'	=> 'Blocos do rodapé',
	'class' 		=> '',
	'before_widget'	=> '<div id="navaux" class="widget %1$s %2$s limpo grid colunas-06">',
	'after_widget'	=> '</div>',
	'before_title'	=> '<h3>',
	'after_title'	=> '</h3>'
) );
...
register_sidebar( array(
	'name'			=> __( 'Quem Somos', 'quem somos' ),
	'id'			=> 'quem somos',
	'description'	=> 'Page quem somos',
	'class' 		=> '',
	'before_widget'	=> '',
	'after_widget'	=> '',
	'before_title'	=> '',
...
register_sidebar( array(
	'name'			=> __( 'Imprensa', 'imprensa' ),
	'id'			=> 'imprensa',
	'description'	=> 'Page de imprensa',
	'class' 		=> '',
	'before_widget'	=> '',
	'after_widget'	=> '',
	'before_title'	=> '',
...
register_sidebar( array(
	'name'			=> __( 'Atendimento', 'atendimento' ),
	'id'			=> 'atendimento',
	'description'	=> 'Page de atendimento',
	'class' 		=> '',
	'before_widget'	=> '',
	'after_widget'	=> '',
	'before_title'	=> '',
...
register_nav_menu( 'principal-top', 'Menu Top' );
...
add_image_size( 'thumbE',			225,	150,	true	);
add_image_size( 'thumbProduto',		200,	175,	true	);
add_image_size( 'thumbBI',			254,	192,	true	);
	...
	register_taxonomy(
		...
		array(
			...
			'labels'							=> array(
				...
				'parent_item'					=> 'Tipos pai',
				'parent_item_colon'				=> 'Tipos pai:',
...
add_action( 'init', 'categoriza_tipo' );
...
function relaciona_type_taxonomia() {
	...
	register_taxonomy_for_object_type( 'tipo', 'produto' );
}
	...
	if( $slides->have_posts() ) {
		...
		while( $slides->have_posts() ) {
			...
			if( has_post_thumbnail() )
				echo get_the_post_thumbnail( get_the_ID(), 'sliderG', array( 'alt' => get_the_title(), 'title' => get_the_title() ) );
			else
	...
	if( $posts->have_posts() ) {
		echo '
			<div id="carrossel" class="limpo">
					<img src="' . get_bloginfo( 'template_directory' ) . '/images/carossel-left.png" class="navegacao esquerda" alt="Voltar" title="Voltar" />
				<div class="visivel">
					<ul class="limpo">
		';
		...
		while( $posts->have_posts() ) {
			...
			echo '<li>';
				if( has_post_thumbnail() )
						echo get_the_post_thumbnail( get_the_ID(), 'thumbG', array( 'alt' => get_the_title(), 'title' => get_the_title() ) );
				else
					echo '<img src="' . get_bloginfo( 'template_directory' ) . '/images/img-thumbG.jpg" alt="' . get_the_title() . '" title="' . get_the_title() . '" />';
			echo '</li>';
		}
		...
		echo '
				...
				<img src="' . get_bloginfo( 'template_directory' ) . '/images/carossel-right.png" class="navegacao direita" alt="Avançar" title="Avançar" />
			</div>
		';
	}
...
function listar_galeria_produto( $id ) {
	...
	if( sizeof( $anexos ) > 1 ) {
		...
		echo '<div class="miniaturas limpo">';
			foreach ( $anexos as $anexo )
				echo wp_get_attachment_image( $anexo->ID, 'thumbProduto', false, array( 'id' => 'thumbProduto', 'class' => 'thumbProduto', 'title' => get_the_title() ) );
		echo '</div>';
		...
		';
				echo '<ul>';
					foreach ( $anexos as $anexo )
						echo '<li>' . wp_get_attachment_image( $anexo->ID, 'sliderP', false, array( 'id' => 'sliderP', 'class' => 'sliderP', 'title' => get_the_title() ) ) . '</li>';
				echo '</ul>';
				echo '
...
function listar_galeria( $id ) {
	...
	if( sizeof( $anexos ) > 1 ) {
		...
		echo '<div class="miniaturas limpo">';
			foreach ( $anexos as $anexo )
				echo wp_get_attachment_image( $anexo->ID, 'thumbE', false, array( 'id' => 'thumbE', 'class' => 'thumbE', 'title' => get_the_title() ) );
		echo '</div>';
...
function listar_galeria_banco_imagens( $id ) {
	$anexos = get_posts( array(
		'post_type'			=> 'attachment',
		'post_status'		=> 'inherit',
		'posts_per_page'	=> -1,
	...
	if( sizeof( $anexos ) > 1 ) {
		...
		echo '<div class="miniaturas limpo">';
			foreach ( $anexos as $anexo )
				echo wp_get_attachment_image( $anexo->ID, 'thumbBI', false, array( 'id' => 'thumbBI', 'class' => 'thumbBI', 'title' => get_the_title() ) );
		echo '</div>';
		...
		';
				echo '<ul>';
					foreach ( $anexos as $anexo )
						echo '<li>' . wp_get_attachment_image( $anexo->ID, 'thumbE', false, array( 'id' => 'thumbE', 'class' => 'thumbE', 'title' => get_the_title() ) ) . '</li>';
				echo '</ul>';
				echo '
					<div class="navegacao limpo">
						<img id="esquerda" src="' . get_bloginfo( 'template_directory' ) . '/images/glr-esquerda.png" />
